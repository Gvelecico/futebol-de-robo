...
from graphics import graficos_xy, graficos_x_t ,graficos_y_t, graficos_vx_t, graficos_vy_t, graficos_d_t,graficos_ax_t,graficos_ay_t  
...
def interface_campo(temp, x, y):
    ...
    while janela_aberta:
        ...
        if(distancia > 10):
            ...
            if(len(vel_x_bola)>=1):
                #tempo_for3.append(temp[i])
                #Pegar velocidade da bola
                ace_x_bola.append((vel_x_bola[-1]- vel_x_bola[0]) / (temp[-1]- temp[0]))
                ace_y_bola.append((vel_y_bola[-1]- vel_y_bola[0]) / (temp[-1]- temp[0]))


                #Pegar velocidade do robo
                ace_x_robo.append((vel_x_robo[-1]- vel_x_robo[0]) / (temp[-1]- temp[0]))
                ace_y_robo.append((vel_y_robo[-1]- vel_y_robo[0]) / (temp[-1]- temp[0]))
    ...
    #Grafico 4
    graficos_ax_t(ace_x_bola, ace_x_robo, tempo_for3)
    graficos_ay_t(ace_y_bola, ace_y_robo, tempo_for3)
